...
def read_predictions(seed, model='tta', tt_split=1, split_type='c_blind'):
    if model == 'mm':
        path = f'results/mm_mets/preds_{split_type}_{tt_split}'
        pred = pd.read_csv(path)['0'] #these preds have a header
    else:
        path = f'results/tt_split_{tt_split}/predictions/{model}/{split_type}_seed{seed}'
        pred = pd.read_csv(path, header=None)[0] #no header for these
    tt_path = f'data/train_test_pairs/{split_type}/seed_{tt_split}_test'
...
def find_mets(seed, model='tta', tt_split=1, split_type='c_blind', 
              strat=False, return_strat=False):
    pred, y_test = read_predictions(seed, model=model, tt_split=tt_split, 
                                    split_type=split_type)
    if strat:
        mets = inference.find_stratifed_mets(pred, y_test, by=strat)
        if return_strat:
            return mets
        mets = mets.mean()
    else:
...
def find_ave_sd(tt_split, split_type='c_blind', strat=False,
    ...
    for model in models:
        if model == 'mm':
            mets = pd.DataFrame(
            [find_mets(None, model, tt_split, split_type, strat)])
        else:
            mets = pd.DataFrame(
                [find_mets(seed, model, tt_split, split_type, strat) 
                 for seed in range(3)])
...
def create_results(tt_split, split_type='c_blind', strat=False, 
    ...
    mets, sd = find_ave_sd(tt_split, split_type, strat, order)
