...
def create_results(tt_split, split_type='c_blind', strat=False,
    ...
    models = [m for m in order if m != 'mm']
    mets, sd = find_ave_sd(tt_split, split_type, strat, models)
    results_table = format_results(mets, sd)
    #results_table = add_bench_mets(results_table, tt_split, split_type, strat=strat)
    if 'mm' in order:
        vals = []
        for val in results_table.loc['mm']:
            dps = 3
            val = str(np.round(val, dps))
